	...
	function upload(file, clone, place, holder, settings)
	{
		...
		xhr.onreadystatechange = function()
		{
			...
			if (xhr.readyState == 4)
			{
				...
				$(clone).html("<div class='picture_delete'>"+settings.orakuploader_file_delete_label+"</div>"+ck_html+rotation_html+"<img src='"+settings.orakuploader_thumbnail_real_path+"/"+xhr.responseText+"' alt='' onerror=this.src='"+settings.orakuploader_no_image+"' class='picture_uploaded'/> <input type='hidden' class='orak_hidden_photo' value='"+xhr.responseText+"' name='"+settings.orakuploader_field_name+"[]' />");
