...
class OrakController extends Controller
{
    ...
    public function upload_file(){
		...
		$thumbnail_path = $_REQUEST["thumbnail_path"];
		...
		{
			$this->rotateImage($_GET['rotate'], $main_path, $_GET['degree_lvl']);
			$this->rotateImage($_GET['rotate'], $thumbnail_path, $_GET['degree_lvl']);
			echo $_GET['rotate'];
			exit;
		}
		...
		$filename = $this->normalizeChars($filename);
		...
		if($_REQUEST["resize_to"] > 0)
		{
			$width = $imgsize[0];
			$height = $imgsize[1];
			if ($width > $_REQUEST["resize_to"]) $this->createThumbnail($main_path, $filename, $main_path, $_REQUEST["resize_to"], 100);
		}

		if($bytes > 8) {
			if((int)$_REQUEST["thumbnail_size"] > 0) $this->createThumbnail($main_path, $filename, $thumbnail_path, $_REQUEST["thumbnail_size"], 100);
		} else exit;

		if(isset($_REQUEST["watermark"]) && $_REQUEST["watermark"] != '')
		{
			$this->addWatermark($dirback.$_REQUEST["watermark"], $main_path, $filename);
			$this->addWatermark($dirback.$_REQUEST["watermark"], $thumbnail_path, $filename);
		}
		...
		{
			$this->crop($crop_thumb_to_width, $crop_thumb_to_height, $main_path.'/'.$filename, $thumbnail_path.'/'.$filename);
		}

		if($crop_to_width > 0 && $crop_to_height > 0)
		{
			$this->crop($crop_to_width, $crop_to_height, $main_path.'/'.$filename, $main_path.'/'.$filename);
		}
	...
	public function createThumbnail($imageDirectory, $imageName, $thumbDirectory, $thumbWidth, $quality = 100)
	{
	...
	public function rotateImage($image_name, $path, $degree_lvl)
	{
		if($degree_lvl == 4)
		{
	...
	public function addWatermark($watermark, $imageDirectory, $imageName, $x = 0, $y = 0)
	{
	...
	public function crop($max_width, $max_height, $source_file, $dst_dir)
	{
