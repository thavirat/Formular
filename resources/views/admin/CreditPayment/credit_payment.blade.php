    ...
    <div class="page-header mb-2 pb-2 flex-column flex-sm-row align-items-start align-items-sm-center py-25 px-1">
        ...
        <div class="page-tools mt-3 mt-sm-0 mb-sm-n1">
             @if( $my_menu_permission[$currentMenu->url]['p'] == 'T' )
                 <a href="{{ url("admin/CreditPayment/ExportPrint") }}" target="_blank" class="btn btn-light-warning btn-h-warning btn-a-warning border-0 radius-3 py-2 text-600 text-90">
            ...
            @if( $my_menu_permission[$currentMenu->url]['ep'] == 'T' )
                <a href="{{ url("admin/CreditPayment/ExportPDF") }}" target="_blank" class="btn btn-light-danger btn-h-danger btn-a-danger border-0 radius-3 py-2 text-600 text-90">
                    <span class="d-none d-sm-inline mr-1">
                        PDF
                    </span>
            ...
            @if( $my_menu_permission[$currentMenu->url]['ee'] == 'T' )
                <a href="{{ url("admin/CreditPayment/ExportExcel") }}" target="_blank" class="btn btn-light-primary btn-h-primary btn-a-primary border-0 radius-3 py-2 text-600 text-90">
                    <span class="d-none d-sm-inline mr-1">
                        Excel
                    </span>
        ...
        <div class="col-12">
            <div class="card dcard">
                <div class="card-body p-0">
                    ...
                    <table id="tableCreditPayment" class="table table-border-x brc-secondary-l4 border-0 mb-0 w-100">
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <form id="FormEdit" data-parsley-validate="true">
                    <input type="hidden" id="CreditPayment_edit_id">
                    <div class="modal-header">
...
<script type="text/javascript">
    ...
    var tableCreditPayment = $('#tableCreditPayment').dataTable({
        scrollY : height-500,
        scrollX : true,
        "ajax": {
            "url": url_gb+"/admin/CreditPayment/Lists",
            "type": "POST",
            "data": function ( d ) {
                // d.status = "A";
    ...
    $('body').on('submit', '#FormAdd', function(e){
        ...
        $.ajax({
            method: "POST",
            url: url_gb+"/admin/CreditPayment",
            dataType : "json",
            data: form.serialize()
        }).done(function( res ) {
            ...
            if(res.status == 1){
                ...
                tableCreditPayment.api().ajax.reload();
                $('#ModalAdd').modal('hide');
            }else{
    ...
    $('body').on('submit', '#FormEdit', function(e){
        e.preventDefault();
        var id = $("#CreditPayment_edit_id").val();
        var form = $(this);
        loadingButton(form.find('button[type=submit]'));
        $.ajax({
            method: "PUT",
            url: url_gb+"/admin/CreditPayment/"+id,
            dataType : 'json',
            data: form.serialize()
            }).done(function( res ) {
                resetButton(form.find('button[type=submit]'));
